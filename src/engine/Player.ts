...
import { GroupObject3D } from "./Object3D/GroupObject3D";
import {
 Projectile3D
} from "./Object3D/Projectile3D";
...
export class Player extends GroupObject3D {
  changeBBColor;
  boundingBoxes;
  enableGravity;
  enablePhysics;
  ...
  constructor(mainMesh, material, projectileMesh, projectileMaterial) {
    ...
    //flashlight
    this.flashlight = new Object3D(Scene.flashlightMesh, Scene.flashlightTex);
  ...
  handleInput() {
    ...
    if (Input.isKeyDown(Input.W_KEY)) {
      this.setSpeed(
        -this.moveSpeed * Math.sin(utils.degToRad(-this.roty)),
        this.speedY,
    ...
    //move left
      ...
      this.setSpeed(
        this.moveSpeed * Math.sin(utils.degToRad(-this.roty - 90)),
        this.speedY,
    ...
    } else this.setSpeed(0, this.speedY, 0);
    ...
    if (Input.isMouseClicked()) {
      ...
      projectile.setSpeed(
        -projectileSpeed * Math.sin(utils.degToRad(-this.roty)),
        -projectileSpeed *
          Math.sin(utils.degToRad(Scene.firstPersonCamera.elevation)),
        -projectileSpeed * Math.cos(utils.degToRad(this.roty)),
      );
      projectile.enableCollisionWith(Scene.objects.slice(1, Scene.objects.length));
  ...
  preUpdate() {
    ...
    if (this.damagedTime >= 30) this.damagedTime = -1;
    else if (this.damagedTime >= 0) this.damagedTime++;

    //regen health
    if (this.health <= 0) this.health = 0;
    else this.health += 0.0003;
